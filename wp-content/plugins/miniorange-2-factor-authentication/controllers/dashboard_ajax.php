...
class Mo2f_ajax_dashboard
{
	...
	public function mo2f_switch_functions(){
			...
			if(current_user_can('administrator') )
			{
		        if(isset($_POST) && isset($_POST['option'])){
					...
					$sanitized_post=isset($_POST['switch_val'])? sanitize_text_field($_POST['switch_val']):null;
					
					switch(sanitize_text_field($_POST['option']))
					{
						case "tab_all_switch":
							if($this->mo2f_check_wpns_nonce()){
						...
						case "tab_waf_switch":
							if($this->mo2f_check_wpns_nonce()){
							$this->mo2f_handle_waf_enable($sanitized_post);
							}
							break;
						case "tab_login_switch":
							if($this->mo2f_check_wpns_nonce()){
							$this->mo2f_handle_login_enable($sanitized_post);
							}
							break;
						case "tab_malware_switch":
							if($this->mo2f_check_wpns_nonce()){
							$this->mo2f_handle_malware_enable($sanitized_post);
							}
							break;
	...
	public function mo2f_check_wpns_nonce(){
		$nonce= isset($_POST['mo2f_wpns_switch_nonce'])?sanitize_text_field($_POST['mo2f_wpns_switch_nonce']):'';
		if(!wp_verify_nonce($nonce,'mo2f-wpns-switch-nonce'))
		{
			$error = new WP_Error();
