...
class mo_2f_ajax
{
			...
			{  
				...
				if($enable=="true")
				{
					...
					if(isset($_POST['mo2f_graceperiod_value']) && $_POST['mo2f_graceperiod_value'] > 0 && $_POST['mo2f_graceperiod_value'] <=10){
						update_option('mo2f_grace_period_value',sanitize_text_field($_POST['mo2f_graceperiod_value']));
					}else{
	...
	{   
		$nonce= isset($_POST['mo2f_otp_sms_and_email_nonce'])?sanitize_text_field($_POST['mo2f_otp_sms_and_email_nonce']):'';
		if(!wp_verify_nonce($nonce,'mo2f-otp-sms-and-email-nonce'))
		{
			$error = new WP_Error();
			$error->add('empty_username', '<strong>'. __('ERROR','miniorange-2-factor-authentication') .'</strong>: '. __('Invalid Request.', 'miniorange-2-factor-authentication'));
			wp_send_json('false');
	...
	function mo2f_save_custom_form_settings()
	{
			...
			sanitize_text_field($_POST['form_selector'])!="")
		{
			...
			update_site_option('mo2f_form_submit_after_validation', $formSubmit);
