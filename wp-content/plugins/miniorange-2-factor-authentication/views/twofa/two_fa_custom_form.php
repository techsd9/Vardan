...
<div>
    ...
    <form name="form_custom_form_config" method="post" action="" id="mo2f_custom_form_config">
        ...
        <div style="padding: 20px;border: 1px #DCDCDC solid">
                ...
                <tr>
                    <td>
                        <select id="regFormList" name="regFormList">
                            ...
                            $formsArray = array("forms"=>array($defaultWordpress,$wcForm,$bbForm,$loginPressForm,$userRegForm,$pmproForm,$customForm));

                            for ($i= 0 ; $i < sizeof($formsArray["forms"]) ; $i++)
                            {
                                $formName = $formsArray["forms"];
                                echo '<option'.($formName[$i]["formSelector"] == get_site_option('mo2f_custom_form_name')?" selected ":'').' value='.strtolower(str_replace(" ","", esc_html($formName[$i]["formName"]))).'>'.esc_html($formName[$i]["formName"]).'</option>';
                                ?>
                                <?php
                            }
                            ?>
            ...
            <br>
            <input type="checkbox" id="use_shortcode_config" name="use_shortcode_config" value="yes" <?php if (get_option('enable_form_shortcode')=='true')echo 'checked';?>>
            <label for="use_shortcode_config">Enable Shortcode</label>
            <input type="checkbox" id="mo2f_form_submit_after_validation" name="mo2f_form_submit_after_validation" value="yes" <?php if (get_option('mo2f_form_submit_after_validation')=='true')echo 'checked';?>>
