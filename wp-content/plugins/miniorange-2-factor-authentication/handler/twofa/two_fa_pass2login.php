	...
	function mo2f_inline_validate_mobile_authentication(){
		if(isset($_POST['mo_auth_inline_mobile_registration_complete_nonce'])){
			...
			if ( ! wp_verify_nonce( $nonce, 'miniorange-2-factor-inline-mobile-registration-complete-nonce' ) ) {
				...
				if($selected_2factor_method == 'SOFT TOKEN'|| $selected_2factor_method == 'miniOrange Soft Token' )
					$selected_2factor_method_onprem = 'miniOrange Soft Token';
				else if($selected_2factor_method == 'PUSH NOTIFICATIONS' || $selected_2factor_method == 'miniOrange Push Notification')
					$selected_2factor_method_onprem = 'miniOrange Push Notification';
				else if($selected_2factor_method == 'MOBILE AUTHENTICATION' ||$selected_2factor_method == 'miniOrange QR Code Authentication')
...
function check_miniorange_duo_push_validation_failed($POSTED){
	...
	function miniorange_initiate_2nd_factor( $currentuser, $attributes = '', $redirect_to = null, $otp_token = "",$session_id_encrypt=null ) {
		global $Mo2fdbQueries,$moWpnsUtility;
		...
		if ( $is_customer_admin && $twofactor_enabled ) {
			...
			}else if(!$exceeded && (MoWpnsUtility::get_mo2f_db_option('mo2f_inline_registration', 'get_option') || $this->mo2f_is_grace_period_expired($currentuser) )){
			...
			if ( is_wp_error( $currentuser ) ) {
				...
				{	         
				      ...
				      {   
				           ...
				           $twofactor_transactions = new Mo2fDB;
		                           $exceeded = $twofactor_transactions->check_alluser_limit_exceeded($currentuser->ID);
		                           if ( get_option('mo2fa_'.$currentuser->roles[0]) && $exceeded==false )
		                           {   
	...
	{	global $mainDir;
		...
		echo "<div  style='background-color: #d5e3d9; height:850px;' >
		    <div style='height:350px; background-color: #3CB371; border-radius: 2px; padding:2%;  '>
		        ...
		        margin-top:10px;' src='".$mainDir . "includes/images/miniorange_logo.png'>
		            <div><hr></div>
