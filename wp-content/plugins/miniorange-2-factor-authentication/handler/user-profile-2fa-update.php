...
$method = MO2f_Utility::mo2f_decode_2_factor($method,'wpdb');
$status=($id==$user_id)?'MO_2_FACTOR_PLUGIN_SETTINGS':'MO_2_FACTOR_INITIALIZE_TWO_FACTOR';
switch ($method) {
    ...
    $Mo2fdbQueries->update_user_details($user_id, array(
        ...
        'mo_2factor_user_registration_status' =>$status,
    ) );
    break;
...
function send_reconfiguration_on_email($email,$user_id,$method){
    ...
    update_option('mo2fa_reconfiguration_via_email',json_encode(array($user_id,$email,$method)));
